...
(defun label-set-text (label str)
  (setf (label-label label) str))

(export 'label-set-text)
...
(defun label-set-attributes (label attrs)
  (setf (label-attributes label) attrs))

(export 'label-set-attributes)
...
(defcfun (%gtk-label-get-selection-bounds "gtk_label_get_selection_bounds")
    :boolean
  (label (g-object label))
  (start (:pointer :int))
  (end (:pointer :int)))
