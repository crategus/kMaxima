...
;;; 
;;; #define             GTK_IS_RESIboZE_CONTAINER(widget)
;;; 
...
;;; 
;;; Returns :
;;; 	a newly-allocated list of the container's non-internal children. [element-type GtkWidget][transfer container]
...
(defun container-get-border-width (container)
  (container-border-width container))

(export 'container-get-border-width)
...
(defun container-set-border-width (container border-width)
  (setf (container-border-width container) border-width))

(export 'container-set-border-width)
...
;;; 
;;; focusable_widgets :
;;; 	location to store the focus chain of the container, or NULL. You should free this list using g_list_free() when you are done with it, however no additional reference count is added to the individual widgets in the focus chain. [element-type GtkWidget][out][transfer container]
;;; 
;;; Returns :
...
;;; 
;;; focusable_widgets :
;;; 	the new focus chain. [transfer none][element-type GtkWidget]
