...
;;; g_spawn_sync() and g_spawn_async_with_pipes(). On POSIX platforms it is
;;; called in the child after GLib has performed all the setup it plans to
;;; perform but before calling exec(). On POSIX actions taken in this function
;;; will thus only affect the child, not the parent.
;;; 
;;; Note that POSIX allows only async-signal-safe functions (see signal(7)) to
;;; be called in the child between fork() and exec(), which drastically limits
;;; the usefulness of child setup functions.
;;; 
;;; Also note that modifying the environment from the child setup function may
;;; not have the intended effect, since it will get overridden by a non-NULL
;;; env argument to the g_spawn... functions.
;;; 
...
;;; name of the program must be a full path; the PATH shell variable will only
;;; be searched if you pass the G_SPAWN_SEARCH_PATH flag. If the program name is
;;; not a full path and G_SPAWN_SEARCH_PATH flag is not used, then the program
;;; will be run from the current directory (or working_directory, if specified);
;;; this might be unexpected or even dangerous in some cases when the current
...
;;; instead of main(). wmain() has a wide character argument vector as
;;; parameter.
;;; 
...
;;; family of functions (which g_spawn_async_with_pipes() eventually calls)
;;; paste the argument vector elements together into a command line, and the C
;;; runtime startup code does a corresponding reconstruction of an argument
;;; vector from the command line, to be passed to main(). Complications arise
;;; when you have argument vector elements that contain spaces of double quotes.
;;; The spawn*() functions don't do any quoting or escaping, but on the other
;;; hand the startup code does do unquoting and unescaping in order to enable
...
;;; will not automatically be reaped; you must use a child watch to be notified
;;; about the death of the child process. Eventually you must call
...
;;; 
;;; G_SPAWN_LEAVE_DESCRIPTORS_OPEN means that the parent's open file descriptors
;;; will be inherited by the child; otherwise all descriptors except
...
;;; be looked for in the user's PATH. G_SPAWN_STDOUT_TO_DEV_NULL means that the
;;; child's standard output will be discarded, instead of going to the same
;;; location as the parent's standard output. If you use this flag,
;;; standard_output must be NULL. G_SPAWN_STDERR_TO_DEV_NULL means that the
...
;;; standard_error must be NULL. G_SPAWN_CHILD_INHERITS_STDIN means that the
...
;;; standard_input must be NULL. G_SPAWN_FILE_AND_ARGV_ZERO means that the first
;;; element of argv is the file to execute, while the remaining elements are the
...
;;; close the handle with CloseHandle() or g_spawn_close_pid() when you no
;;; longer need it.
;;; 
;;; If non-NULL, the standard_input, standard_output, standard_error locations
...
;;; g_spawn_async_with_pipes() must close these file descriptors when they are
;;; no longer in use. If these parameters are NULL, the corresponding pipe won't
;;; be created.
;;; 
...
;;; graphical application, too, then you may want to use
;;; gdk_spawn_on_screen_with_pipes() instead to ensure that the spawned program
;;; opens its windows on the right screen.
;;; 
;;; working_directory :
;;; 	child's current working directory, or NULL to inherit parent's, in the
;;;     GLib file name encoding. [allow-none]
;;; 
;;; argv :
;;; 	child's argument vector, in the GLib file name encoding.
;;; 
;;; envp :
;;; 	child's environment, or NULL to inherit parent's, in the GLib file name
;;;     encoding. [array zero-terminated=1][allow-none]
;;; 
;;; flags :
...
;;; 
;;; child_pid :
;;; 	return location for child process ID, or NULL. [out][allow-none]
;;; 
;;; standard_input :
...
;;; 
;;; If you are writing a GTK+ application, and the program you are spawning is a
;;; graphical application, too, then you may want to use gdk_spawn_on_screen()
;;; instead to ensure that the spawned program opens its windows on the right
;;; screen.
;;; 
;;; Note
;;; 
;;; Note that the returned child_pid on Windows is a handle to the child process
;;; and not its identifier. Process handles and process identifiers are
;;; different concepts on Windows.
;;; 
;;; working_directory :
...
;;; NULL for standard_output and standard_error. If exit_status is non-NULL, the
;;; exit status of the child is stored there as it would be returned by
;;; waitpid(); standard UNIX macros such as WIFEXITED() and WEXITSTATUS() must
;;; be used to evaluate the exit status. Note that this function call waitpid()
;;; even if exit_status is NULL, and does not accept the
;;; G_SPAWN_DO_NOT_REAP_CHILD flag. If an error occurs, no data is returned in
;;; standard_output, standard_error, or exit_status.
;;; 
...
;;; 
;;; exit_status :
;;; 	return location for child exit status, as returned by waitpid(), or
;;;     NULL. [out][allow-none]
;;; 
;;; error :
...
;;; Windows paths, like "c:\\program files\\app\\app.exe", as the backslashes
;;; will be eaten, and the space will act as a separator. You need to enclose
